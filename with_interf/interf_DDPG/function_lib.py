...
def sir_cal(cb, H, inter_1):
    ...
    ch_r = H[:, :n_ant]
    ch_i = H[:, n_ant:]
    ch_i1_r = inter_1[:, :n_ant]
    ch_i1_i = inter_1[:, n_ant:]
    ...
    bf_gain_i1_1 = torch.matmul(bf_r, torch.t(ch_i1_r))
    bf_gain_i1_2 = torch.matmul(bf_i, torch.t(ch_i1_i))
    bf_gain_i1_3 = torch.matmul(bf_r, torch.t(ch_i1_i))
    bf_gain_i1_4 = torch.matmul(bf_i, torch.t(ch_i1_r))
    bf_gain_i1_r = (bf_gain_i1_1 + bf_gain_i1_2) ** 2
    bf_gain_i1_i = (bf_gain_i1_3 - bf_gain_i1_4) ** 2
    bf_gain_i1_pattern = bf_gain_i1_r + bf_gain_i1_i  # BF gain pattern matrix
    bf_gain_i1 = torch.mean(bf_gain_i1_pattern)
    
    return (bf_gain_pattern/bf_gain_i1)
...
def corr_mining(bf_gain_matrix):
    ...
    mat_1 = torch.zeros((num_feature, num_user)).float().cpu()
    mat_2 = torch.zeros((num_feature, num_user)).float().cpu()
