...
class TD3(object):
    ...
    def select_action_invase(self, state, action):
        state = torch.FloatTensor(state.reshape(1, -1)).to(device)
        action = torch.FloatTensor(action.reshape(1, -1)).to(device)
        return self.invase_actor(state,action).cpu().data.numpy().flatten()


    def train(self, replay_memory, batch_size=100, lmda = 0.0, thr = 0.0): # lmd, thr: parameters for generator training
        self.total_it += 1
    ...
    def save(self, filename):
        ...
        torch.save(self.baseline.state_dict(), filename + "_baseline")
        torch.save(self.baseline_optimizer.state_dict(), filename + "_baseline_optimizer")

        torch.save(self.actor.state_dict(), filename + "_actor")
        torch.save(self.actor_optimizer.state_dict(), filename + "_actor_optimizer")

        torch.save(self.invase_actor.state_dict(), filename + "_invase_actor")
        torch.save(self.invase_optimizer.state_dict(), filename + "_invase_optimizer")
    ...
    def load(self, filename):
        ...
        self.baseline.load_state_dict(torch.load(filename + "_baseline"))
        self.baseline_optimizer.load_state_dict(torch.load(filename + "_baseline_optimizer"))
        self.baseline_target = copy.deepcopy(self.baseline)
        ...
        self.invase_actor.load_state_dict(torch.load(filename + "_invase_actor"))
        self.invase_optimizer.load_state_dict(torch.load(filename + "_invase_optimizer"))
